...
void ImagePainter::drawPixel(OUT Image* image, IN uint32_t pixelPosition, IN uint32_t color) {
    if (pixelPosition >= image->imageWidth * image->imageHeight)
        return;
    image->image[pixelPosition] = color;
}

void ImagePainter::drawPixels(OUT Image* image, IN uint32_t startPixelPosition, IN uint32_t endPixelPosition, IN uint32_t color) {
